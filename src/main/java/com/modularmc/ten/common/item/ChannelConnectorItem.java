 ...
 * int[6] DescSynced 数组 + 推送同步；channelId 非空则目标 join</li>
...
public class ChannelConnectorItem extends TENBaseItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!(be instanceof AbstractChannelBlockEntity channel)) {
            ...
            stack.remove(TENDataComponents.CHANNEL_CONFIG.get());
        ...
        // 潜行右键频道方块且未持有组件 → 复制
        if (!stack.has(TENDataComponents.CHANNEL_CONFIG.get())) {
            if (level.isClientSide()) {
                return InteractionResult.SUCCESS;
            }
            stack.set(TENDataComponents.CHANNEL_CONFIG.get(), copyFrom(channel));
            player.sendSystemMessage(ComponentHelper.translated(ChatFormatting.GREEN,
        ...
        ChannelConfigComponent config = stack.get(TENDataComponents.CHANNEL_CONFIG.get());
        if (config != null) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown() && stack.has(TENDataComponents.CHANNEL_CONFIG.get())) {
            if (level.isClientSide()) {
