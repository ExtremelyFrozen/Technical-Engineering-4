...
### Fixed
- **AE2 样板供应器阻挡模式失效**（机器/频道机的面配为非可抽取面时，如主动输出、被动输入、关闭）：面 capability 的观察读（getStackInSlot/getFluidInTank）被外部抽取权限一并藏空，AE2 判定目标机器为空 → 阻挡模式失效、连发配方；改为观察读真实反映机器内容（写入/抽取仍由面权限单独门控）
- **配置面板钮对齐**：内容钮坐标系原点误取纹理内图形原点（=叠加层纹理原点 + 纹理 5px 原生留白），留白被计两次，6 个面配置钮与 3 个类型钮整体右下偏 5px、脱离 `panel_config_legacy.png` 的钮槽；改用叠加层纹理原点后与底图逐像素对齐
