...
- 计划路径: `plans/plan_engine_model_migration.md`
- 版本状态: `已批准`
- 触发原因: `审查修订`
- 建议下一步: `审查通过，进入执行`
- 审查结论: `猫娘审查官-艾琳` 复审通过。批准前完成修订：SSH 前置验证、分支 A/B 决策标准、纹理映射表强制输出、路径修正、DataGen 预检查（TASK-000）、范围澄清、远程仓库不可访问回退补充。修订后计划满足可执行、可验证、可回退要求。
...
- 输出:
  - **适配方案文档**（记录以下决策）：
    ...
    - blockstate 是否沿用现有 Registrate blockstate 生成逻辑
...
- 验收要点:
  - 每引擎的 idle 正面纹理按映射表已到位
  - 每引擎的 active 正面纹理按映射表已到位
  - 每引擎的 active 动画 .mcmeta 按映射表已到位
  - 每引擎的底面纹理按映射表已到位
...
- 验收要点:
  - 使用 TASK-000 确认的 DataGen task 名称运行，无编译错误、无运行异常、无资源缺失警告
...
本计划当前不含子计划，无需拆分原子化计划。若 TASK-002 分支决策后某 TASK 需要独立子计划推进，后续在此挂接。
