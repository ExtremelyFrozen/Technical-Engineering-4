...
- 版本状态: `草案`
- 触发原因: 审计发现16项未完成迁移残项，用户要求"逐个攻破"切换到任务模式
- 建议下一步: `委托细化` → 审查
...
- 工作区状态: dirty（dependencies.gradle + forge.versions.toml 已修改；untracked 4 文件）
...
2. DoD 可量化可验证：静态验证 → compile → GameTest → runData diff → clean build → runClient人工
...
- 验收要点:
  ...
  - [ ] 修改某项配置值 → 重启后值持久化
  - [ ] `ugrep "ConfigHolder\.init" src/` 确认已移除或仅为空方法
...
- 验收要点:
  - [ ] `.\gradlew.bat :compileJava` 零错误
  - [ ] `ugrep "ResourceHandler.*ItemResource" src/main/java/com/modularmc/ten/api/capability/CapabilityAdapters.java` 匹配新方法
...
- 验收要点:
  - [ ] `ugrep "implements IEnergyStorage\|extends IEnergyStorage" src/main/java/` 无内部类实现（仅保留 `EnergyUnitItem` 中的匿名类作为遗留适配层，加 `// LEGACY-ADAPTER` 注释）
  - [ ] `.\gradlew.bat :compileJava` 零错误
- 输入: 当前能量存储实现
- 输出: 迁移后的能量存储代码
- 依赖: TASK-033（边界适配已验证）
...
- 验收要点:
  - [ ] `ugrep "new ItemStackHandler\|extends ItemStackHandler" src/main/java/` 无创建旧实例的代码（桥接层除外）
  - [ ] `.\gradlew.bat :compileJava` 零错误
- 输入: 当前物品存储实现
- 输出: 迁移后的物品存储代码
...
- 验收要点:
  - [ ] `ugrep "IFluidHandler\|FluidTank" src/main/java/` 无新实现（桥接除外）
  - [ ] `.\gradlew.bat :compileJava` 零错误
- 输入: 当前流体存储实现
- 输出: 迁移后的流体存储代码
...
- 验收要点:
  ...
  - [ ] `ugrep "TODO.*filter inventory NBT" src/main/java/com/modularmc/ten/common/blockentity/PipeBlockEntity.java` 无输出
...
- 验收要点:
  ...
  - [ ] 在下界放置：始终不产生
  - [ ] `ugrep "TODO.*day-only" src/main/java/` 无输出
...
- 验收要点:
  ...
  - [ ] `ugrep "TODO.*Material variants" src/main/java/` 无输出
...
- 验收要点:
  ...
  - [ ] `ugrep "TODO.*Curios" src/main/java/` 无输出
...
- 验收要点:
  - [ ] `runClient` 日志 `select-string "FluidModel.*kenergyengineering"` 无输出
...
> 最终阶段：fresh clean build、GameTest 运行、runData diff、runClient 人工场景、完整验证矩阵。
...
- DoD:
  - □ `.\gradlew.bat clean :build 2>&1` 退出码 0
  - □ `build/libs/` 产出 `kenergyengineering-*.jar`，大小 > 100KB
  - □ jar 内 `META-INF/neoforge.mods.toml` 内容正确（无 configuration 引用）
...
- DoD:
  ...
  - □ `.\gradlew.bat :compileTestJava 2>&1` 退出码 0（零测试编译错误）
  - □ `.\gradlew.bat :compileJava :compileTestJava 2>&1` 退出码 0
...
- DoD:
  ...
  - □ `.\gradlew.bat :runGameTestServer 2>&1` 运行完成，`Passed: X` > 0，`Failed: 0`
...
- 目标: 验证 datagen 完整运行且 generated 输出合理
- DoD:
  - □ `.\gradlew.bat :runClientData 2>&1` 退出码 0
...
| R09 | FluidModel WARN 为 NeoForge 内部行为不可消除 | P7 | 低 | 低 | 确认非 blocking 后记录 evidence 关闭 |
| R10 | TagsProvider 标准 API 输出 diff 不可控 | P8 | 低 | 低 | 保留自定义 DataProvider，TODO 标注 |
...
| 文件 | 用途 | 关联 TASK |
|------|------|-----------|
| `plans/.evidence/evidence_26_1_2_migration_completion_01.md` | NeoForge 26.1.2 ResourceHandler / Capabilities / Transaction / ModConfigSpec 参考 | P2-P4 |
| `plans/.evidence/evidence_26_1_2_migration_completion_02.md` | Curios Modrinth 构件确认 / EMI 官方版本检查 | P6 |
