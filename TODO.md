...
用户追加
配置展开面出现了两层io配置按钮和类型选择按钮，导致更新按倍增后被另一层按钮覆盖显示，且额外的一层还具备完整的tooltip
扳手的功能全部失效：
 右键旋转方块
 潜行右键破坏含有机器标签的方块
 右键管理管道状态
升级展开面的物品被槽位遮盖，物品仅在放入的一瞬间可见
管道过滤槽需要从放入式过滤改为标记式过滤
过滤过滤槽放入物品的瞬间会出现物品槽位遮盖，且当此GUI中物品持续被遮盖，直到下一次打开GUI
生物啃噬者的造成伤害应该和内部的物品的攻击力有关，且工作周期与内部物品攻击速度有关，无物品时伤害和间隔为玩家空手的数值，批处理会提升目标数量
对应开关附魔效果应用的按钮，不需要在开启时赘述支持的附魔，对应附魔的应用，需要直接套用附魔的逻辑，而不是硬穷举支持的附魔带来的参数变化
机器的IO限制中，物品流量和流体流量不再执行限制，且需要完善的梳理机器的IO的执行与配置
频道系统的配置tab上移到对齐机器信息tab
管道系统移植后，现有的能量管道失去了能量上限
在补充了c：模具的标签后旧的ker：模具未删除
