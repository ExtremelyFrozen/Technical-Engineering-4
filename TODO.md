...
用户追加
升级面板和配置面板的空位点击收起功能，收敛空位的概念，因为升级面板和配置面板新增有一个中间叠加层，现在需要这个点击这个中间叠加层的范围内也不会收起
配置面板我发现：中间tab和九宫格底图的内边框间距过大，调整间距与升级面板的与内边框间距相同
关于九宫格展开动画，部分人反馈，过慢，增加一个neoforge类型的配置文件，用与动态调整每一个展开面板的展开动画速度，使用统一函数+独立函数的混合算法，并且可以调整为关闭动画，且配置项的tooltip和配置项本身的文本需要本地化
部分物品注册矩阵的物品缺失标签
部分模具数据包配方中模具缺失不消耗修饰符
频道系统存在一些问题：
1.连接了频道的频道面板，被破坏时，会将反馈替换为未连接频道的频道面板，而不是直接破坏
2.偶现能量频道在连接了频道且被输入能量时，打开GUI会导致输出：打开带有高级数据的屏幕失败
3.关于频道共享容量，目前出现了被破坏了的频道面板仍然为频道网络提供共享上限，可能需要检查共享逻辑