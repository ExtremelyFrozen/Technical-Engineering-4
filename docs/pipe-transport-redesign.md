# 管道传输重构 — 逐级传递模型（已实现，化繁为简）

- 日期：见 git 提交
- 范围：Kenergy Engineering（NeoForge 26.1.2），物品管道 `pipe / pipe_white / pipe_black`
- 阶段：已实现（提交见 git log；逐级传递 + 统一 64/tick + 机器 IO 生命周期）
- 状态：✅ 已实现（本文件含代码改动描述）

---

> **2.0 重构摘要（化繁为简）**：移除升级系统与网络 root 撮合/主动被动互补，改为
> **每根管道独立 tick 的逐级传递**（Pull 到内部缓冲 64/tick → Push 到相邻容器/管道，
> 物品逐跳移动）；机器面配置六个状态语义显式化（无/被动输入/被动输出/被动双向/
> 主动输入/主动输出），主动 IO 由机器自身 tick 执行（64/tick），被动面仅允许外部操作。
> 面配置客户端镜像完整同步（图标即时更新）。
>
> **交互边界**：管道仅与机器的**被动面**（被动输入/被动输出/被动双向）交互——
> 抽机器需被动输出/被动双向面，塞机器需被动输入/被动双向面；机器的主动输入/主动输出
> 由机器自身对**相邻容器**执行（不跨管道）。能量/流体面配置的 IN/OUT 暂保持被动语义
> （可被外部塞入/抽取），未实现主动拉推——与物品的主动/被动语义存在差异，属已知边界。
